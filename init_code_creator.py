...
def CURRENTTITLE():
    ...
    return render_template("tutorials/CURRENTTOPIC/CURRENTHTML", curLink = TOPIC_DICT["CURRENTTOPIC"][CURRENTINDEX][1], curTitle=TOPIC_DICT["CURRENTTOPIC"][CURRENTINDEX][0],  nextLink = TOPIC_DICT["CURRENTTOPIC"][NEXTINDEX][1], nextTitle = TOPIC_DICT["CURRENTTOPIC"][NEXTINDEX][0],LEVEL_1_DICT = LEVEL_1_DICT)
...
for each_topic in TOPIC_DICT:
    ...
    for eachele in TOPIC_DICT[each_topic]:
        try:
            ...
            CURRENTTITLE = eachele[0].replace("-","_").replace(" ","_").replace(",","").replace("/","").replace(")","").replace("(","").replace(".","").replace("!","").replace(":","_").replace("'","").replace("?","").replace("3D","_3D").replace("__","_")
